...
{
    /// <summary>
    /// Although FPC seems to claim rather good results in native versions,
    /// it does not seem to compare well with other managed codecs we test.
    /// TODO: Figure out if we are doing something WRONG with this algorithm.
    /// </summary>
    /// <remarks>
    /// FPC currently only handles double floating point series.
    /// </remarks>
    public sealed class FpcCodec : DeltaCodec<NullTransform>
    {
        ...
        /// irrelevant, so it doesn't matter what is specified for those. Thus, we don't declare
        /// this constructor with the internal access modifier because it would serve no purpose.
        /// </summary>
        ...
        static FpcCodec()
        {
            Instance = new FpcCodec();
        }

        public static FpcCodec Instance { get; protected set; }
